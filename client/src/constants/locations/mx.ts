export const MEXICAN_CITIES = [
    '31 de Octubre',
    'Abasolo',
    'Abasolo del Valle',
    'Abraham González',
    'Acacoyagua',
    'Acámbaro',
    'Acambay',
    'Acamilpa',
    'Acamixtla',
    'Acanceh',
    'Acapetahua',
    'Acapetlahuaya',
    'Acaponeta',
    'Acapulco',
    'Acatic',
    'Acatlán',
    'Acatlán de Juárez',
    'Acatlán de Osorio',
    'Acatlán de Pérez Figueroa',
    'Acatzingo',
    'Acaxochitlán',
    'Acaxtlahuacán de Albino Zertuche',
    'Acayucan',
    'Achotal',
    'Aconchi',
    'Actopan',
    'Acuchillán del Progreso',
    'Acuitzeramo',
    'Acuitzio del Canje',
    'Acula',
    'Aculco',
    'Acultzingo',
    'Adjuntas del Río',
    'Adolfo López M. (El Jahuara )',
    'Adolfo Ruiz Cortines',
    'Agostitlán',
    'Agua Blanca',
    'Agua Blanca Iturbide',
    'Agua Buena',
    'Agua Caliente de Garate',
    'Agua Dulce',
    'Agua Dulce Papantla',
    'Agua Prieta',
    'Agua Verde',
    'Agualeguas',
    'Aguaruto',
    'Aguascalientes',
    'Aguilera',
    'Aguililla',
    'Ahome',
    'Ahuacatitlán Centro',
    'Ahuacatlán',
    'Ahuacuotzingo',
    'Ahualulco',
    'Ahuazotepec',
    'Ahuehuepan',
    'Ajacuba',
    'Ajalpan',
    'Ajijic',
    'Akil',
    'Akumal',
    'Alamo Temapache',
    'Alamos',
    'Alcozauca de Guerrero',
    'Alfajayucan',
    'Alfonso G. Calderón (Pob. 7)',
    'Alfonso G. Calderón Velarde',
    'Alfredo B. Bonfil',
    'Alfredo V. Bonfil',
    'Allende',
    'Almagres',
    'Almoloya de Alquisiras',
    'Almoloya de Juárez',
    'Almoloya de las Granadas',
    'Alpoyeca',
    'Altamira',
    'Altamirano',
    'Altar',
    'Altata',
    'Alto Lucero',
    'Altotonga',
    'Altzayanca',
    'Alvarado',
    'Alvaro Obregón',
    'Amacueca',
    'Amacuzac',
    'Amanalco de Becerra',
    'Amapa',
    'Amarillas de Esparza',
    'Amartillán',
    'Amatepec',
    'Amatitán',
    'Amatlán',
    'Amatlán de Cañas',
    'Amayuca',
    'Amealco',
    'Ameca',
    'Amecameca',
    'Ameche',
    'Amozoc',
    'Amuco de la Reforma',
    'Anacleto Canabal 1a. Sección',
    'Anáhuac',
    'Angamacutiro',
    'Angangueo',
    'Angel Rosario Cabada',
    'Angostura',
    'Anonas',
    'Antiguo Morelos',
    'Anton Lizardo',
    'Antonio Amaro',
    'Antonio Escobedo',
    'Antunez',
    'Apan',
    'Apango',
    'Apaseo El Alto',
    'Apaseo El Grande',
    'Apatzingán',
    'Apaxco',
    'Apaxtla de Castrejón',
    'Apizaco',
    'Aporo',
    'Apulco',
    'Aquiles Serdán (San Fernando)',
    'Aquismón',
    'Aramberri',
    'Arandas',
    'Aratichanguio',
    'Arcelia',
    'Areponapuchi',
    'Ario de Rayón',
    'Ario de Rosales',
    'Arivechi',
    'Arizpe',
    'Arperos',
    'Arreguín de Abajo',
    'Arriaga',
    'Arroyo Grande',
    'Arroyo Hondo',
    'Arroyo Seco',
    'Arteaga',
    'Asientos',
    'Asunción Ixtaltepec',
    'Asunción Nochixtlán',
    'Atacheo de Regalado',
    'Atemajac de Brizuela',
    'Atenango del Río',
    'Atencingo',
    'Atengo',
    'Atenguillo',
    'Atenquique',
    'Atequiza',
    'Aticama',
    'Atil',
    'Atitalaquia',
    'Atlacomulco',
    'Atlapexco',
    'Atlatlahuca',
    'Atlatlahucán',
    'Atliaca',
    'Atlixco',
    'Atlixtac',
    'Atolinga',
    'Atongo de Abajo',
    'Atotonilco de Tula',
    'Atotonilco El Alto',
    'Atotonilco El Grande',
    'Atotonilco (santuario)',
    'Atoyac de Alvarez',
    'Atoyatempan',
    'Atzitzintla',
    'Autlán',
    'Axochiapan',
    'Axotlán',
    'Axtla de Terrazas',
    'Ayotlán',
    'Ayotoxco de Guerrero',
    'Ayutla',
    'Ayutla de los Libres',
    'Ayutla (San Felipe Ayutla)',
    'Azoyu',
    'Baborigame',
    'Baca',
    'Bacadehuachi',
    'Bacalar',
    'Bacame Nuevo',
    'Bacerac',
    'Bachiniva',
    'Bachoco',
    'Bachomobampo Número Dos',
    'Bacoachi',
    'Bacobampo',
    'Bacum',
    'Badiraguato',
    'Bagojo Colectivo E. Zapata',
    'Bahía Bufadero',
    'Bahía de Huatulco(crucesita)',
    'Bahía de Kino',
    'Bahía Tortugas',
    'Bajamar',
    'Bajío de Bonillas',
    'Bajío de San José',
    'Bajos de Chila',
    'Bajos del Ejido',
    'Balancán',
    'Banamichi',
    'Banderilla',
    'Barra Vieja',
    'Barranca de Otates',
    'Barranca de Santa Clara',
    'Barrio El Rosario',
    'Base Aérea Militar No. 5',
    'Baviacora',
    'Bavispe',
    'Becal',
    'Belén del Refugio',
    'Bella Vista del Río',
    'Bellavista',
    'Benemérito Juárez',
    'Benito Juárez',
    'Benito Juárez (Mpio.B. Juárez)',
    'Benito Juárez (Mpio.Buenaven',
    'Benito Juárez (Mpio.Sombrerete)',
    'Benito Juárez (Tecolotes)',
    'Benjamin Hill',
    'Bermejillo',
    'Berriozabal',
    'Betania',
    'Betulia',
    'Boca de Camichin',
    'Boca del Monte',
    'Boca del Río',
    'Bochil',
    'Bocoyna',
    'Bomintzha',
    'Boquilla de Arriba',
    'Boye',
    'Buaysiacobe',
    'Bucerías',
    'Buctzotz',
    'Buena Vista',
    'Buenavista',
    'Buenavista de Cortés',
    'Buenavista de Cuellar',
    'Buenavista Tomatlán',
    'Buenos Aires',
    'Buenos Aires(Zinacántal)',
    'Burgos',
    'Bustamante',
    'Cabañas Juluapan',
    'Cabeza de Toro',
    'Cabo del Sol',
    'Cabo Real',
    'Cabo San Lucas',
    'Caborca',
    'Cacahoatán',
    'Cacalchen',
    'Cadereyta',
    'Caimanero',
    'Cajititlán',
    'Calderitas',
    'Calera de Víctor Rosales',
    'Caleras de Ameche',
    'Calimaya de Díaz González',
    'Calipan',
    'Calkini',
    'Calmeca (San Juan Calmeca)',
    'Calnali',
    'Calpulalpan',
    'Caltimacán',
    'Calvillo',
    'Calzada del Tepozán',
    'Camalu',
    'Camarón de Tejeda',
    'Camotlán de Miraflores',
    'Campamento Sarh',
    'Campeche',
    'Campo 303',
    'Campo 305 ( Jagueyes )',
    'Campo Acosta',
    'Campo Balbuena',
    'Campo Cinco (La Honda)',
    'Campo Cuichapa (Mpio. Cuichapa)',
    'Campo Diecisiete (La Honda)',
    'Campo Menonita Número 106',
    'Campo Militar N. 37B (El Cabi)',
    'Campo Numero Ciento Uno Viejo',
    'Campo Número Cientosiete',
    'Campo Número Ocho',
    'Campo Número Seis y Medio B',
    'Campo Número Veinte',
    'Campo Número Veintidós',
    'Canalejas',
    'Cananea',
    'Canatlán',
    'Cancún',
    'Candela',
    'Candelaria',
    'Canelas',
    'Cansahcab',
    'Cánticas',
    'Canutillo',
    'Cañada de Caracheo',
    'Cañada del Tabaco',
    'Cañadas de Obregón',
    'Cañitas de Felipe Pescador',
    'Cap. Felipe Castellanos',
    'Capacho',
    'Capardillas (Rcho Sta Lugarda)',
    'Capilla de Guadalupe',
    'Capilla de Milpillas',
    'Carácuaro',
    'Carapán',
    'Carbo',
    'Cárdenas',
    'Careyes',
    'Carichi',
    'Carlos A. Carrillo',
    'Carmen',
    'Carrizal Grande',
    'Casa Blanca',
    'Casa de Janos',
    'Casas',
    'Casas Grandes',
    'Casimiro Castillo',
    'Casitas',
    'Castaños',
    'Castillo de Teayo',
    'Catalinas (Francisco Villa)',
    'Catazaja',
    'Catemaco',
    'Caucel',
    'Caurio de Guadalupe (Cabrio)',
    'Cazones de Herrera',
    'Cd. Cuauhtémoc',
    'Ceballos',
    'Cedral',
    'Celaya',
    'Celestun',
    'Cenobio Moreno (Las Colonias)',
    'Cenotillo',
    'Cerano (San Juan Cerano)',
    'Cerralvo',
    'Cerrillos (Campo 35)',
    'Cerrito Colorado',
    'Cerritos',
    'Cerritos de Villa de Pozos',
    'Cerro Azul',
    'Cerro Colorado',
    'Cerro de Ortega',
    'Cerro Gordo (San Rafael)',
    'Chable',
    'Chacaltianguis',
    'Chahuites',
    'Chalcatzingo',
    'Chalchihuites',
    'Chalco',
    'Chalma',
    'Chamácuaro',
    'Chametla',
    'Champotón',
    'Chamula',
    'Changata',
    'Chapala',
    'Chapalilla',
    'Chapantongo',
    'Chapulhuacán',
    'Chapulhuacanito',
    'Charay',
    'Charcas',
    'Charco Blanco',
    'Charco de Pantoja',
    'Charo',
    'Chaucingo',
    'Chavinda',
    'Chemax',
    'Chemuyil',
    'Cherán',
    'Cherán Atzicuirin',
    'Chetumal',
    'Chiapa',
    'Chiapa de Corzo',
    'Chiautempan',
    'Chiautla de Tapia',
    'Chicavasco',
    'Chichicaxtle',
    'Chichihualco',
    'Chichimequillas',
    'Chicomuselo',
    'Chiconcuac de Juárez',
    'Chiconquiaco',
    'Chicontepec',
    'Chicxulub Pueblo',
    'Chignahuapan',
    'Chihuahua',
    'Chihuahuita',
    'Chijol 17',
    'Chila de la Sal',
    'Chila de las Flores',
    'Chilapa',
    'Chilchota',
    'Chilcuautla',
    'Chilón',
    'Chilpancingo',
    'Chimalhuacán',
    'Chimaltitán',
    'China',
    'Chinameca',
    'Chinantla',
    'Chinotahueca',
    'Chipilo de Francisco Javier M.',
    'Chiquilistlán',
    'Chocamán',
    'Chochola',
    'Choix',
    'Cholul',
    'Cholula de Rivadabia',
    'Chontla',
    'Chosto de los Jarros',
    'Chucándiro',
    'Chunhuhub',
    'Chupicuaro (Nuevo Chupicuaro)',
    'Churintzio',
    'Cibuta',
    'Ciénaga de Flores',
    'Ciénaga de Ntra. Señora de Gpe.',
    'Ciénaga Grande',
    'Cieneguillas',
    'Cihuatlán',
    'Cintalapa',
    'Citala',
    'Citlaltepec',
    'Ciudad Acuña',
    'Ciudad Altamirano',
    'Ciudad Anáhuac',
    'Ciudad Apodaca',
    'Ciudad Benito Juárez',
    'Ciudad Camargo',
    'Ciudad Cardel',
    'Ciudad Constitución',
    'Ciudad Cuauhtémoc',
    'Ciudad de Armería',
    'Ciudad de Huajuapam de León',
    'Ciudad de México',
    'Ciudad de Villa de Alvarez',
    'Ciudad del Carmen',
    'Ciudad del Maíz',
    'Ciudad Delicias',
    'Ciudad General Escobedo',
    'Ciudad Guzmán',
    'Ciudad Hidalgo',
    'Ciudad Jerez de García Salinas',
    'Ciudad Jiménez',
    'Ciudad Juárez',
    'Ciudad Lázaro Cárdenas',
    'Ciudad Lerdo',
    'Ciudad López Mateos',
    'Ciudad Madera',
    'Ciudad Madero',
    'Ciudad Mante',
    'Ciudad Manuel Doblado',
    'Ciudad Mendoza',
    'Ciudad Mier',
    'Ciudad Miguel Alemán',
    'Ciudad Morelos (Cuervos)',
    'Ciudad Nezahualcoyotl',
    'Ciudad Obregón',
    'Ciudad Quetzalcoatl',
    'Ciudad Sahagún',
    'Ciudad Santa Catarina',
    'Ciudad Serdán',
    'Ciudad Valles',
    'Ciudad Victoria',
    'Club de Golf Santa Anita',
    'Coacoatzintla',
    'Coacoyul',
    'Coacoyula de Alvarez',
    'Coahuayana de Hidalgo',
    'Coahuayana Viejo',
    'Coajomulco',
    'Coalcoman',
    'Coamiles',
    'Coatepec',
    'Coatepec Harinas',
    'Coatetelco',
    'Coatlán del Río',
    'Coatzacoalcos',
    'Coatzingo',
    'Coatzintla',
    'Cocotitlán',
    'Cocula',
    'Coeneo de la Libertad',
    'Cofradía',
    'Cofradía de Suchitlán',
    'Cohuibampo',
    'Cojumatlán',
    'Col. 28 de Mayo (Santa Rosa)',
    'Col. Agricola Independencia',
    'Col. Agrícola México',
    'Col. Benito García(Zorrillo)',
    'Colesio',
    'Colima',
    'Colipa',
    'Colombia',
    'Colon',
    'Colonia 18 de Marzo',
    'Colonia Alvaro Obregón (Rubio)',
    'Colonia Anáhuac',
    'Colonia Campesina',
    'Colonia El Meco',
    'Colonia El Tajo',
    'Colonia Gonzalez Ortega',
    'Colonia Hidalgo',
    'Colonia Irrigación',
    'Colonia Jecopaco',
    'Colonia Juárez',
    'Colonia La Puerta',
    'Colonia Le Barón',
    'Colonia Madero',
    'Colonia Madero (Madero)',
    'Colonia Manuel González',
    'Colonia Nuevo Morelos',
    'Colonia Piloto',
    'Colonia Soconusco',
    'Colonia Vicente Guerrero',
    'Colonia Yucatán',
    'Colorines',
    'Colotlan',
    'Colotlipa',
    'Colucán (San Lucas Colucán)',
    'Comala',
    'Comalcalco',
    'Comalteco',
    'Comanja',
    'Comanjilla',
    'Comitán de Domínguez',
    'Comoapan',
    'Comonfort',
    'Compostela',
    'Concepción de Buenos Aires',
    'Concepción del Oro',
    'Concordia',
    'Conduacán',
    'Coneto de Comonfort',
    'Congregación Calles',
    'Conkal',
    'Contepec',
    'Copainala',
    'Copala',
    'Copalillo',
    'Copandaro',
    'Coquimatlán',
    'Corazón de María',
    'Córdoba',
    'Corerepe (El Gallo)',
    'Coronel Gregorio Mendez Magaña',
    'Coroneo',
    'Corozal',
    'Corral Nuevo',
    'Corralejo de Hidalgo',
    'Cortazar',
    'Corte Primero',
    'Corupo',
    'Cosala',
    'Cosamaloapan',
    'Cosautlán de Carvajal',
    'Coscomatepec',
    'Cosío',
    'Cosoleacaque',
    'Costa Rica',
    'Cotaxtla',
    'Cotija de la Paz',
    'Covadonga',
    'Coxcatlán',
    'Coyame',
    'Coyotepec',
    'Coyotitán',
    'Coyuca de Benitez',
    'Coyuca de Catalán',
    'Coyuquilla Norte',
    'Coyutla',
    'Cozumel',
    'Creel',
    'Cristóbal Obregón',
    'Cruz Azul San Miguel Vindho',
    'Cruz de Juanacaxtle',
    'Cruz Grande',
    'Cruz Quemada',
    'Cuachila',
    'Cuahuchichinola',
    'Cuajinicuilapa',
    'Cualac',
    'Cuamío',
    'Cuanajo',
    'Cuatrocienegas',
    'Cuauhtémoc',
    'Cuautepec de Hinojosa',
    'Cuautitlán',
    'Cuautitlán Izcalli',
    'Cuautla',
    'Cuautlapan',
    'Cubilete',
    'Cubiri de Portelas',
    'Cuéncame de Ceniceros',
    'Cuerámaro',
    'Cuernavaca',
    'Cuetzala del Progreso',
    'Cuetzalán',
    'Cuevas de Morales',
    'Cuicatlán',
    'Cuichapa (Mpio. Moloacan)',
    'Cuitláhuac',
    'Cuitzeo del Porvenir',
    'Culiacán',
    'Culiacancito',
    'Cumpas',
    'Cumuatillo',
    'Cupareo',
    'Cuquio',
    'Curimeo',
    'Cuto del Porvenir',
    'Cutzamala de Pinzón',
    'Cuyoaco',
    'Cuyutlán',
    'Degollado',
    'Delta (Estación Delta)',
    'Dieciocho de Marzo',
    'Diez de Octubre',
    'Dimas (Estación Dimas)',
    'División del Norte',
    'Doctor Arroyo',
    'Doctor Coss',
    'Doctor González',
    'Dolores',
    'Dolores Hidalgo',
    'Donato Guerra',
    'Dongú',
    'Dos Arroyos',
    'Dos Ríos',
    'Dr. Mora',
    'Dr. Porfirio Parra (La Caseta)',
    'Durango',
    'Dzibalchen',
    'Dzidzantun',
    'Dzilam de Bravo',
    'Dzilam González',
    'Dzitbalche',
    'Dziuche',
    'Ebano',
    'Ecatepec de Morelos',
    'Ecuandureo',
    'Ej. Dr. Alberto Oviedo M.(Indiv',
    'Ejido 27 de Enero',
    'Ejido B.Juárez (Mpio.Namiquipa)',
    'Ejido Chiapas 1',
    'Ejido Constitución',
    'Ejido Cuauhtémoc (Campo Cinco)',
    'Ejido Distrito Federal',
    'Ejido El Largo',
    'Ejido El Quemadito',
    'Ejido Francisco Javier Mina',
    'Ejido General Leandro Valle',
    'Ejido Germinal',
    'Ejido Guadalajara',
    'Ejido Guanajuato',
    'Ejido José María Morelos',
    'Ejido La Victoria',
    'Ejido Lagunitas',
    'Ejido Lázaro Cárdenas',
    'Ejido Monterrey',
    'Ejido Ojo de Agua',
    'Ejido Querétaro',
    'Ejido Quintana Roo',
    'Ejido Revolución (Las Víboras)',
    'Ejido Sanson Flores',
    'Ejido Sinaloa',
    'Ejido Yucatán',
    'Ejutla',
    'Ejutla de Crespo',
    'El Abra',
    'El Ahuaje',
    'El Alazán',
    'El Alberto',
    'El Arenal',
    'El Barretal',
    'El Barrial',
    'El Barrio de la Soledad',
    'El Bolillo',
    'El Bordo Buenavista',
    'El Burrión',
    'El Camarón',
    'El Capomo',
    'El Capulín',
    'El Cargadero',
    'El Carmen',
    'El Castillo (Mpio. Mazatlán)',
    'El Castillo (Mpio. Navolato)',
    'El Cazadero',
    'El Cerro Cabezón',
    'El Chante',
    'El Chauz',
    'El Chique',
    'El Chote',
    'El Chucarit',
    'El Cocuite',
    'El Colomo',
    'El Compás',
    'El Conchal',
    'El Control',
    'El Coyote',
    'El Derramadero',
    'El Descanso',
    'El Dorado',
    'El Escondido',
    'El Espinal',
    'El Espinalillo',
    'El Farallón(Campamento CFE)',
    'El Fortín',
    'El Fresno',
    'El Fuerte',
    'El Grullo',
    'El Higo',
    'El Hongo',
    'El Huitussi',
    'El Ingenio Azucarero',
    'El Jabalí',
    'El Jazmín',
    'El Jícaro',
    'El Jicote',
    'El Jobo',
    'El Jocoque',
    'El Limón',
    'El Lucero (Arcinas)',
    'El Magozal',
    'El Mármol',
    'El Molino',
    'El Moralillo',
    'El Naranjo',
    'El Nayar',
    'El Nigromante',
    'El Oasis Valsequillo',
    'El Ocotito',
    'El Oro',
    'El Palmar',
    'El Paraíso',
    'El Parral',
    'El Pedregal San José',
    'El Pitahayal',
    'El Porvenir',
    'El Porvenir Guadalupe',
    'El Porvenir (Mpio. Bachiniva)',
    'El Porvenir (Mpio. Praxedis G.)',
    'El Potrero',
    'El Potrero de Sataya',
    'El Pozole',
    'El Pueblito',
    'El Puesto',
    'El Quelite',
    'El Ramillete',
    'El Rayón',
    'El Roble',
    'El Rosario',
    'El Sabino',
    'El Salado',
    'El Salitre',
    'El Salitre Palmarillos',
    'El Salto',
    'El Salvador',
    'El Saucito',
    'El Sauz',
    'El Sauz de Abajo',
    'El Sauzal',
    'El Sauz(Villaseñor)',
    'El Tajuelo',
    'El Tephe',
    'El Tigre',
    'El Tizate',
    'El Tothie',
    'El Trapiche',
    'El Triunfo',
    'El Tuito',
    'El Tule',
    'El Venadillo',
    'El Verde',
    'El Vergel (Ejido El Vergel)',
    'El Zacatón',
    'Emiliano Zapata',
    'Emiliano Zapata (Morones)',
    'Emilio Carranza',
    'Empalme',
    'Empalme Escobedo',
    'Encarnación de Díaz',
    'Ensenada',
    'Entabladero',
    'Entronque Congregación Hgo.',
    'Epazoyucan',
    'Epitacio Huerta',
    'Erendira',
    'Ermita de Guadalupe',
    'Ermita de los Correas',
    'Erongaicuaro',
    'Escalón',
    'Escárcega',
    'Escuinapa',
    'Escuintla',
    'Españita',
    'Esperanza',
    'Espinal',
    'Espita',
    'Esqueda',
    'Estación Bamoa (Campo Wilson)',
    'Estación Catorce',
    'Estación Chontalpa',
    'Estación Coahuila (Km 57)',
    'Estación de Apulco',
    'Estación Llano',
    'Estación Naranjo',
    'Estación Queréndaro',
    'Estación San Diego',
    'Estación San José',
    'Estación Santa Engracia',
    'Estación Yago',
    'Estancia de Animas',
    'Estanzuela',
    'Estero de la Bocana',
    'Estipac',
    'Etchojoa',
    'Etchoropo',
    'Etucuaro',
    'Etzatlán',
    'Exhacienda de Jesús María',
    'Ex-hacienda de Solís',
    'Ezequiel Montes',
    'Felipe Carrillo Puerto',
    'Felipe Neri (Cuatepec)',
    'Flamboyanes',
    'Fortín de las Flores',
    'Fortines y Emiliano Zapata',
    'Fracc. Emiliano Zapata',
    'Fracc. Industrial Bernardo Q.',
    'Fracc. Kunetzin de los Volcan',
    'Francisco I. Madero',
    'Francisco I. Madero (Puga)',
    'Francisco Javier Mina',
    'Francisco R. Serrano',
    'Francisco Sarabia',
    'Franco Tavera',
    'Fresnillo',
    'Frías',
    'Frontera',
    'Frontera Comalapa',
    'Fronteras',
    'Fuentes del Valle',
    'Gabriel Leyva Solanoj',
    'Gabriel Zamora (Lombardia)',
    'Galeana',
    'Gambara',
    'Gandho',
    'García',
    'Garza Ayala',
    'Garza González',
    'Genaro Estrada',
    'General  Chávez Talamantes',
    'General Bravo',
    'General Cepeda',
    'General Enrique Estrada',
    'General Escobedo',
    'General Francisco Villa',
    'General Genaro Codina',
    'General Ignacio Zaragoza',
    'General Jesús Agustín Castro',
    'General Pedro María Anaya',
    'General Simón Bolívar',
    'General Terán',
    'General Treviño',
    'General Zaragoza',
    'General Zuazua',
    'Gildardo Magaña (Los Angeles)',
    'Golfo de Santa Clara',
    'Gómez Farías',
    'Gómez Palacio',
    'González',
    'González Ortega',
    'Graciano Sánchez',
    'Gral. Fco. González Villarreal',
    'Gral. J. Ma. Morelos y Pavón',
    'Gral.Felipe Carrillo Puerto',
    'Granjas Residenciales',
    'G.Rodrigo M. Quevedo (Pto.P.)',
    'Gsemez',
    'Guachinango',
    'Guachochi',
    'Guadalajara',
    'Guadalupe',
    'Guadalupe de Rivera',
    'Guadalupe Distrito Bravos',
    'Guadalupe (Guadalupe de Ures)',
    'Guadalupe Santa Ana',
    'Guadalupe Victoria',
    'Guadalupe Victoria (Km 43)',
    'Guadalupe Victoria (Mp.Asención)',
    'Guadalupe Victoria (Mp.Meoqui)',
    'Guadalupe y Calvo',
    'Guamuchil',
    'Guanacevi',
    'Guanajuato',
    'Guarapo',
    'Guardados de Arriba',
    'Guasave',
    'Guascuaro de Mugica',
    'Guatimape',
    'Guayameo',
    'Guaymas',
    'Guegovela',
    'Guerrero',
    'Guerrero Negro',
    'Gustavo Díaz O. (El Carrizo)',
    'Gustavo Díaz Ordaz',
    'Gutiérrez Zamora',
    'Hacienda de Cabañas',
    'Hacienda Nueva',
    'Halacho',
    'Hecelchakan',
    'Hermenegildo Galeana',
    'Hermosillo',
    'Hernández',
    'Héroes de la Independencia',
    'Hidalgo',
    'Hidalgo (Colonia Hidalgo)',
    'Hidalgo (Ejido Hidalgo)',
    'Hidalgotitlán',
    'Higuera de Zaragoza',
    'Higueras',
    'Hocaba',
    'Hoctun',
    'Holbox',
    'Honey',
    'Hopelchen',
    'Hostotipaquillo',
    'Huachinantla',
    'Huachinera',
    'Huajicori',
    'Huajintlán',
    'Huajumbaro',
    'Hualahuises',
    'Huamantla',
    'Huamuxtitlán',
    'Huandacareo',
    'Huanimaro',
    'Huaniqueo de Morales',
    'Huanusco',
    'Huapango',
    'Huasabas',
    'Huasca de Ocampo',
    'Huascato',
    'Huatabampo',
    'Huatlatlauca',
    'Huatusco',
    'Huauchinango',
    'Huautla',
    'Huautla de Jiménez',
    'Huayacocotla',
    'Huayalejo',
    'Huecahuasco',
    'Huehuetán',
    'Huehuetlán El Chico',
    'Huehuetoca',
    'Huejotzingo',
    'Huejucar',
    'Huejuquilla El Alto',
    'Huejutla de Reyes',
    'Huepac',
    'Huetamo',
    'Hueyotlipan',
    'Hueypoxtla',
    'Hueytamalco',
    'Hueytepec',
    'Huhi',
    'Huichapan',
    'Huichihuayan',
    'Huimanguillo',
    'Huimilpan',
    'Huiramba',
    'Huisquilco (Huiscuilco)',
    'Huitchila',
    'Huitzatarito',
    'Huitzilac',
    'Huitzuco',
    'Huixquilucan',
    'Huixtac',
    'Huixtla',
    'Hunucma',
    'Ibarra',
    'Idolos',
    'Ignacio Allende',
    'Ignacio de la Llave',
    'Ignacio Ramírez',
    'Ignacio Zaragoza',
    'Iguala',
    'Ihuatzio',
    'Imuris',
    'Indaparapeo',
    'Inde',
    'Independencia',
    'Independencia (Cologachi)',
    'Infiernillo',
    'Ingeniero Francisco Murguía',
    'Ingeniero Luis B. Sánchez',
    'Ingenio Alvaro Obregón',
    'Iramuco',
    'Irapuato',
    'Isla',
    'Isla Aguada',
    'Isla de Cedros',
    'Isla del Bosque',
    'Isla Mujeres',
    'Islas Agrarias Grupo A',
    'Islas Agrarias Grupo B',
    'Islita',
    'Iturbide',
    'Ixcamilpa',
    'Ixcapuzalco',
    'Ixcateopan de Cuauhtémoc',
    'Ixcatepec',
    'Ixhuatlán del Café',
    'Ixhuatlán del Sureste',
    'Ixmiquilpan',
    'Ixtacomitán',
    'Ixtaczoquitlán',
    'Ixtahuiata (La Legua)',
    'Ixtapa',
    'Ixtapa de la Concepción',
    'Ixtapa Zihuatanejo',
    'Ixtapaluca',
    'Ixtapan de la Sal',
    'Ixtapan del Oro',
    'Ixtepec',
    'Ixtlahuaca',
    'Ixtlahuacán',
    'Ixtlahuacán de los Membrillos',
    'Ixtlahuacán del Río',
    'Ixtlán de Juárez',
    'Ixtlán de los Hervores',
    'Ixtlán del Río',
    'Izamal',
    'Izúcar de Matamoros',
    'Jacala',
    'Jacona de Plancarte',
    'Jagüey de Téllez',
    'Jala',
    'Jalacingo',
    'Jalapa',
    'Jalapa del Marque',
    'Jalapa (Xalapa)',
    'Jalcocotán',
    'Jalostotitlán',
    'Jalpa',
    'Jalpa de Méndez',
    'Jalpan de Serra',
    'Jaltenango de la Paz',
    'Jaltepec',
    'Jaltiche de Abajo',
    'Jaltipán',
    'Jaltomate',
    'Jamapa',
    'Jamay',
    'Janambo',
    'Janos',
    'Jarácuaro',
    'Jaral del Progreso',
    'Jaripo',
    'Jaumave',
    'Jecopaco',
    'Jerécuaro',
    'Jesús Carranza',
    'Jesús María',
    'Jesús María Garza',
    'Jilotepec',
    'Jilotzingo',
    'Jiménez',
    'Jiménez del Teul',
    'Jiquilpan de Juárez',
    'Jiquipilas',
    'Jiquipilco',
    'Jitotol de Zaragoza',
    'Jiutepec',
    'Joachín',
    'Jocotepec',
    'Jocotitlán',
    'Jojutla',
    'Jolalpan',
    'Jonacatepec',
    'Jonuta',
    'Jorobas',
    'José Cardel',
    'José Guadalupe Aguilera',
    'José Guadalupe Rodríguez',
    'José María Morelos',
    'José María Morelos (San José)',
    'José María Pino Suárez',
    'Josefino de Allende',
    'Juan Aldama',
    'Juan Aldama (El Tigre)',
    'Juan Díaz Covarrubias',
    'Juan José Ríos',
    'Juan Lucas',
    'Juan Martín',
    'Juanita',
    'Juárez',
    'Jucatacato',
    'Juchipila',
    'Juchique de Ferrer',
    'Juchitán',
    'Juchitepec',
    'Juchitlán',
    'Julimes',
    'Jumiltepec',
    'Jungapeo',
    'Jurica (Ex-Hacienda)',
    'Juriquilla',
    'Juventino Rosas',
    'Kanasin',
    'Kantunil',
    'Kantunilkin',
    'Kaua',
    'Kilometro 30',
    'Komchen',
    'La Aldea',
    'La Antigua',
    'La Ascensión',
    'La Barca',
    'La Boquilla del Conchos',
    'La Brecha',
    'La Bufadora',
    'La Caja',
    'La Calera',
    'La Calera (El Canario)',
    'La Cañada',
    'La Caridad (Fracción G)',
    'La Choya',
    'La Ciudad',
    'La Concepción',
    'La Concha',
    'La Concordia (Rosita)',
    'La Constancia',
    'La Cruz',
    'La Encarnación',
    'La Escondida',
    'La Esperanza',
    'La Estancia',
    'La Estancia de Sn José del Carmen',
    'La Estanzuela',
    'La Excusa',
    'La Frontera',
    'La Fuente',
    'La Garita',
    'La Goma',
    'La Higuera',
    'La Huacana',
    'La Huerta',
    'La Joya',
    'La Joya de Calvillo',
    'La Junta',
    'La Laguna y Monte del Castillo',
    'La Lagunilla',
    'La Libertad (Mpio. La Libertad)',
    'La Libertad (Mpio. Suchiate)',
    'La Loma',
    'La Luz',
    'La Luz (Mpio. Pajacuarán)',
    'La Luz (Mpio. Penjamillo)',
    'La Magdalena Tetela Morelos',
    'La Manzanilla',
    'La Manzanilla de la Paz',
    'La Mesa (El Fresnito)',
    'La Mira',
    'La Noria',
    'La Noria Hidalgo',
    'La Norteña',
    'La Palma (La Palma de Jesús)',
    'La Palma (Las Palmas)',
    'La Panadera',
    'La Partida',
    'La Paz',
    'La Perla',
    'La Pesca',
    'La Piedad',
    'La Plaza de Fco. J. Mugica',
    'La Presa',
    'La Providencia',
    'La Punta',
    'La Purísima',
    'La Reforma',
    'La Ribera',
    'La Sauceda',
    'La Soledad',
    'La Tinaja',
    'La Trinidad',
    'La Trinitaria',
    'La Unidad Huitzizilapan',
    'La Unión',
    'La Valla',
    'La Venta',
    'La Venta (Cacaloxuchitl)',
    'La Venta del Astillero',
    'La Ventosa',
    'La Victoria',
    'La Vigueta',
    'La Yerbabuena',
    'Lagos de Moreno',
    'Laguna del Rey (Del Rey',
    'Laguna Grande',
    'Laguna Larga',
    'Lagunas',
    'Lagunillas',
    'Lagunillas de Rayón',
    'Lampazos',
    'Las Cabañas',
    'Las Cañadas',
    'Las Cañas',
    'Las Choapas',
    'Las Cieneguitas',
    'Las Coloradas',
    'Las Esperanzas',
    'Las Esperanzas (El Ranchito)',
    'Las Garzas',
    'Las Grullas Margen Izquierda',
    'Las Guacamayas',
    'Las Hilamas (El Piedrero)',
    'Las Jicamas',
    'Las Lagunitas',
    'Las Margaritas',
    'Las Mesas',
    'Las Navajas',
    'Las Nieves',
    'Las Palmas de Arriba',
    'Las Pintas',
    'Las Plazas',
    'Las Ranas',
    'Las Varas',
    'Las Vigas',
    'Las Virginias',
    'Lázaro Cárdenas',
    'Lázaro Cárdenas (Mp.Cuauhtémoc)',
    'Lázaro Cárdenas (Mpio Meoqui)',
    'León',
    'León Fonseca',
    'Leona Vicario',
    'Lequeitio',
    'Lerdo de Tejada',
    'Lerma',
    'Ley Federal de Reforma Agraria',
    'Libres',
    'Limón de los Ramos',
    'Linares',
    'Linda Vista',
    'Llano Grande',
    'Llera de Canales',
    'Lo Arado',
    'Lo de Juárez',
    'Lo de Marcos',
    'Loma Alta',
    'Loma Bonita',
    'Loma de Zempoala',
    'Lomas de Arena',
    'Lomas del Real',
    'Lombardo de Caso',
    'Loreto',
    'Loreto (Teresa)',
    'Los Abanicos (El Abanico)',
    'Los Aldamas',
    'Los Altos',
    'Los Ayala',
    'Los Aztecas',
    'Los Azulitos',
    'Los Campos',
    'Los Charcos',
    'Los Colorados de Abajo',
    'Los Comales',
    'Los Frailes',
    'Los Globos',
    'Los Guajes',
    'Los Herrera',
    'Los Herreras',
    'Los Limones',
    'Los Mochis',
    'Los Ocotes',
    'Los Pilares',
    'Los Ramones',
    'Los Reyes',
    'Los Reyes Acaquilpan',
    'Los Reyes Acozac',
    'Los Reyes de Juárez',
    'Los Robles',
    'Los Rodríguez',
    'Los Sauces',
    'Los Tecomates (Piedra Pesada)',
    'Los Tepames',
    'Los Tigres (San Marcos)',
    'Los Volcanes',
    'Loza de los Padres (La Loza)',
    'Luces del Mar',
    'Luchana',
    'Luis Moya',
    'Luvianos',
    'Macultepec-Ocuiltzapotlan',
    'Macuspana',
    'Madrid',
    'Magdalena',
    'Magdalena Chichicaspa',
    'Magdalena de Kino',
    'Magdalena Teitipac',
    'Magdalena Tequisistlán',
    'Maguey',
    'Mahuixtlán',
    'Malinalco',
    'Malpaso',
    'Maltrata',
    'Manalisco',
    'Maneadero',
    'Mani',
    'Manlio Fabio Altamirano',
    'Manuel Benavides',
    'Manuel M. Dieguez',
    'Manuel Ojinaga',
    'Manzanillo',
    'Mapastepec',
    'Mapimi',
    'Maravatío',
    'Maravillas',
    'Marcelino Rodríguez',
    'Marena Cove',
    'Marfil',
    'Margaritas',
    'Mariano Escobedo (San Lorenzo)',
    'Mariano Matamoros',
    'Marín',
    'Mariscala de Juárez',
    'Marquelia',
    'Marte R. Gómez (Tobarito)',
    'Martínez de la Torre',
    'Martiniano Hernández',
    'Mascota',
    'Matachic',
    'Matamoros',
    'Matanzas',
    'Matatlán',
    'Matehuala',
    'Matías Romero',
    'Matlapa',
    'Maxcanú',
    'Maxela',
    'Mayanalán',
    'Mazamitla',
    'Mazapil',
    'Mazatán',
    'Mazatecochco',
    'Mazatlán',
    'Mecapalapa',
    'Mecatán',
    'Mechoacanejo',
    'Medellín de Bravo',
    'Medias Aguas',
    'Medina',
    'Melchor Ocampo',
    'Melchor Ocampo (El Salto)',
    'Mérida',
    'Metapa de Domínguez',
    'Metepec',
    'Metlaltoyuca',
    'Metztitlán',
    'Mexicali',
    'Mexicaltzingo',
    'Mexquitic de Carmona',
    'Mexticacán',
    'Mezcala',
    'Mezcala de los Romeros',
    'Mezcala (Mp.Tepatitlán de Morelo)',
    'Mezcales',
    'Mezquital del Oro',
    'Mezquite Alto',
    'Mezquitic',
    'Mezquititlán',
    'Miacatlán',
    'Miahuatlán',
    'Michapa',
    'Michoacán de Ocampo',
    'Miguel Alemán (La Doce)',
    'Miguel Auza',
    'Milpas Viejas',
    'Mina',
    'Minas de Barroterán',
    'Minatitlán',
    'Mineral de San Pedro Pozos',
    'Mineral del Monte',
    'Miquihuana',
    'Miraflores',
    'Misantla',
    'Misión',
    'Mismaloya',
    'Mitla',
    'Mixquiahuala',
    'Mixtlán',
    'Mochicahui Pueblo',
    'Mochitlán',
    'Mocorito',
    'Moctezuma',
    'Molango',
    'Molcaxac',
    'Momax',
    'Moncada',
    'Monclova',
    'Monte Blanco',
    'Monte de Peña',
    'Monte Escobedo',
    'Monte Mariana',
    'Monte Verde (El Coyote)',
    'Montemorelos',
    'Monteón',
    'Monterrey',
    'Morelia',
    'Morelos',
    'Morelos Cañada',
    'Morocoy',
    'Moroleón',
    'Motozintla',
    'Motul',
    'Moyahua',
    'Mozomboa',
    'Mulege (Mulege)',
    'Muna',
    'Muñoz',
    'Muzquiz',
    'Nacajuca',
    'Nacionalista de Sánchez T.',
    'Naco',
    'Nacori Chico',
    'Nacozari',
    'Naica',
    'Nanacamilpa',
    'Nanchital de Lázaro Cárdenas',
    'Naolinco de Victoria',
    'Napateco',
    'Naranjal',
    'Naranjos',
    'Naucalpan de Juárez',
    'Nautla',
    'Nava',
    'Navarrete',
    'Navojoa',
    'Navolato',
    'Nayarit Llamada',
    'Nazareno',
    'Nazas',
    'Necaxa',
    'Nepantla de Sor Juana Inés de',
    'Neutla',
    'Nextlalpan',
    'Nicolás Bravo',
    'Nicolás R. Casillas',
    'Nieves',
    'Nigromante',
    'Nio',
    'Nochistlán',
    'Nogales',
    'Nombre de Dios',
    'Nonoava',
    'Nopala',
    'Nopaltepec',
    'Nopolo',
    'Noria de Angeles',
    'Nueva Ciudad Guerrero',
    'Nueva Concordia',
    'Nueva Italia',
    'Nueva Madera',
    'Nueva Rosita',
    'Nueve de Diciembre',
    'Nuevo  Padilla',
    'Nuevo Balsas',
    'Nuevo Casas Grandes',
    'Nuevo Ideal',
    'Nuevo Ixcatlán',
    'Nuevo Laredo',
    'Nuevo León',
    'Nuevo México',
    'Nuevo Progreso',
    'Nuevo Progreso (El Doce)',
    'Nuevo San Diego Tensaenz',
    'Nuevo San Juan Parangaricutiro',
    'Nuevo Tantoán',
    'Nuevo Urecho',
    'Nuevo Vallarta',
    'Nuevo Xcan',
    'Nuevo Zirosto',
    'Numerán',
    'Nurio',
    'Nuxco',
    'Oacalco',
    'Oaxaca de Juárez',
    'Oaxtepec',
    'Ocampo',
    'Ochomitas',
    'Oconahua',
    'Ocosingo',
    'Ocotepec',
    'Ocotlán',
    'Ocotlán de Morelos',
    'Ocoyoacac',
    'Ocozocoautla',
    'Ocuilán de Arteaga',
    'Ojo de Agua',
    'Ojo de Agua de Ballesteros',
    'Ojo de Agua de Palmillas',
    'Ojocaliente',
    'Ojos Negros Real del Castillo',
    'Ojuelos de Jalisco',
    'Olinalá',
    'Omealca',
    'Ometepec',
    'Omitlán de Juárez',
    'Once de Febrero 1a. Sección',
    'Ordeñita de Barajas',
    'Orduña de Arriba',
    'Oriental',
    'Orizaba',
    'Orizabita',
    'Oscar Soto Maynez',
    'Otaez',
    'Otatitlán',
    'Otatlán',
    'Otumba',
    'Oxchuc',
    'Oxkutzcab',
    'Oxtotipac',
    'Oxtotitlán',
    'Ozuluama',
    'Ozumba',
    'Ozumbilla (Ejido El Zapote)',
    'Pabellón de Arteaga',
    'Pabellón de Hidalgo',
    'Pacho Viejo',
    'Pachuca de Soto',
    'Pahuatlán de Valle',
    'Pajarearán',
    'Palau',
    'Palenque',
    'Palizada',
    'Palma de la Cruz',
    'Palma Grande',
    'Palma Pegada',
    'Palma Sola',
    'Palmar Chico',
    'Palmar de Bravo',
    'Palmarito Tochapan',
    'Palo Alto',
    'Palo Alto de Abajo',
    'Palo Verde',
    'Palomar',
    'Palomares',
    'Palos Altos',
    'Palos Blancos',
    'Panaba',
    'Pánfilo Natera',
    'Panindicuaro',
    'Panotla',
    'Pantanal',
    'Pantelho',
    'Pánuco',
    'Pánuco de Coronado',
    'Papaloapam',
    'Papalotla',
    'Papanoa',
    'Papantla',
    'Paracho',
    'Parácuaro',
    'Paraíso',
    'Paraíso La Reforma',
    'Paraíso Novillero',
    'Paraje Nuevo',
    'Paredón',
    'Paredones',
    'Pareo',
    'Parícuaro',
    'Parque Ind. Cienega de Flores',
    'Parque Industrial Cd. Mitras',
    'Parral',
    'Parras de la Fuente',
    'Parrilla Primera Sección',
    'Pascual Orozco (San Isidro)',
    'Paso Ancho',
    'Paso de Arena',
    'Paso de Cuarenta',
    'Paso de Ovejas',
    'Paso del Macho',
    'Paso Morelos (Cuetlajuchi)',
    'Paso Nacional',
    'Pastor Ortiz',
    'Pastoria',
    'Patambán (Patambam)',
    'Pátzcuaro',
    'Patzímaro de Aviña',
    'Pedernales',
    'Pedriceña (Estación Pedriceña)',
    'Pedro Amaro',
    'Pedro Escobedo',
    'Pedro Meoqui',
    'Pegueros',
    'Pemex (Ciudad Pemex)',
    'Penjamillo',
    'Pénjamo',
    'Peña Blanca',
    'Peñon Blanco',
    'Peñuelas',
    'Peralta',
    'Peribán de Ramos',
    'Pericos',
    'Perote',
    'Pesqueira',
    'Pesqueria',
    'Petacalco',
    'Petalcingo',
    'Petatlán',
    'Petlalcingo',
    'Peto',
    'Pichucalco',
    'Pie de Gallo',
    'Piedras Negras',
    'Pihuamo',
    'Pijijiapan',
    'Pilotos',
    'Pimienta',
    'Pinal de Amoles',
    'Pinos',
    'Pinotepa de Don Luis',
    'Pinotepa Nacional',
    'Piñicuaro',
    'Pipioltepec',
    'Pisaflores Hidalgo',
    'Piste',
    'Pitiquito',
    'Pizándaro',
    'Placeres del Oro',
    'Plan de Arroyos',
    'Plan de Ayala',
    'Plan de Ayala (Santa Rosa)',
    'Plan de Iguala',
    'Plan de las Hayas',
    'Platón Sánchez',
    'Playa Azul',
    'Playa del Carmen',
    'Playa Vicente',
    'Playas del Rosario',
    'Plenitud',
    'Poblado Héroes de Chapultepec',
    'Poblado Número Cinco',
    'Poblado Número Seis',
    'Polotitlán',
    'Pomuch',
    'Ponciano Arriaga',
    'Poncitlán',
    'Popotla',
    'Potam',
    'Potrerillo del Norote',
    'Potrerillos',
    'Potrero',
    'Poza Rica',
    'Pozo de Gamboa',
    'Pozo de Ibarra',
    'Pozos CD. Porfirio Díaz',
    'Praderas de la Hacienda',
    'Praderas del Potrero',
    'Praxedis G. Guerrero',
    'Primero de Mayo',
    'Primo Tapia',
    'Progreso',
    'Progreso de Alfonso Medina',
    'Progreso Industrial',
    'Providencia',
    'Puebla',
    'Pueblito de Allende',
    'Pueblo Juárez (La Magdalena)',
    'Pueblo Mayo',
    'Pueblo Nuevo',
    'Pueblo Nuevo Comaltitlán',
    'Pueblo Nuevo Solistahuacán',
    'Pueblo Sánchez Magallane',
    'Pueblo Tecamac',
    'Pueblo Viejo',
    'Pueblo Yaqui',
    'Pueblos Unidos',
    'Puente de Ixtla',
    'Puente Grande',
    'Puentecillas',
    'Puerta de Andaracua',
    'Puerta de Palmillas',
    'Puerto Adolfo López Mateos',
    'Puerto Angel',
    'Puerto Arista',
    'Puerto Aventuras',
    'Puerto Escondido',
    'Puerto Libertad',
    'Puerto Madero (San Benito)',
    'Puerto Morelos',
    'Puerto Peñasco',
    'Puerto San Carlos',
    'Puerto Vallarta',
    'Pujal Coy',
    'Pujiltic',
    'Punta Abreojos',
    'Punta Banda',
    'Punta Cólonet',
    'Punta del Agua',
    'Purépero',
    'Purísima de Bustos',
    'Puruándiro',
    'Puruarán',
    'Putla Villa de Guerrero',
    'Quebrantadero (San José)',
    'Quechultenango',
    'Queréndaro',
    'Querétaro',
    'Querobabi',
    'Quesería',
    'Quetchehueca',
    'Quila',
    'Quila (Quila El Grande)',
    'Quimichis',
    'Quiringuicharo (La Hacienda)',
    'Quiroga',
    'Quitupán',
    'Rafael Ramirez (Las Vigas)',
    'Ramón Corona',
    'Ramos Arizpe',
    'Rancho Avándaro',
    'Rancho Contento',
    'Rancho Grande',
    'Rancho La Tijera',
    'Rancho Nuevo',
    'Rancho Nuevo de la Cruz',
    'Rancho Nuevo de San Andrés',
    'Rancho Seco de Guantes',
    'Rascón',
    'Raudales Malpaso',
    'Rayón',
    'Real de Catorce',
    'Reforma',
    'Refugio',
    'República Mexicana',
    'Revolución Mexicana',
    'Reynosa',
    'Ricardo Flores Magón',
    'Rincón de Guayabitos',
    'Rincón de López',
    'Rincón de Romos',
    'Rincón de Tamayo',
    'Rincón del Porvenir',
    'Rincón Juárez',
    'Rinconada',
    'Río Blanco',
    'Río Bravo',
    'Río Florido',
    'Río Frío',
    'Río Grande',
    'Río Lagartos',
    'Río Laja',
    'Rioverde',
    'Rodeo',
    'Rodrigo M. Quevedo',
    'Rodríguez Clara',
    'Romita',
    'Rosamorada',
    'Rosario',
    'Rosario Tesopaco',
    'Rosarito (Campo 30-06)',
    'Ruben Jaramillo',
    'Ruiz',
    'Rumorosa',
    'Sabana Grande',
    'Sabancuy',
    'Sabinas',
    'Sabinas Hidalgo',
    'Sahuaripa',
    'Sahuayo',
    'Sain Alto',
    'Saladero',
    'Saladillo (El Saladillo)',
    'Salamanca',
    'Salazar',
    'Saldarriaga',
    'Salina Cruz',
    'Salinas de Hidalgo',
    'Salinas Victoria',
    'Salitral de Carrera',
    'Saltillo',
    'Salto de Agua',
    'Salto de Tepuxtepec',
    'Salvatierra',
    'Samalayuca',
    'San Agustín Amatengo',
    'San Agustin Etla',
    'San Agustín Tlaxiaca',
    'San Agustín Zapotlán',
    'San Andrés Calpan',
    'San Andrés Cholula',
    'San Andrés Coru',
    'San Andrés Cuexcontitlán',
    'San Andrés Figueroa',
    'San Andrés Huaxpaltepec',
    'San Andrés La Venta',
    'San Andrés Timilpan',
    'San Andrés Tlalamac',
    'San Andrés Tuxtla',
    'San Andrés Zirondaro',
    'San Antonio Atotonilco',
    'San Antonio Chiltepec',
    'San Antonio de Fernández',
    'San Antonio de los Vázquez',
    'San Antonio de Mogotes',
    'San Antonio del Mar',
    'San Antonio del Rosario',
    'San Antonio Guaracha',
    'San Antonio La Isla',
    'San Antonio Ocampo',
    'San Antonio Puerta de la Vega',
    'San Antonio Rayón',
    'San Antonio Tecomitl',
    'San Antonio Tenextepec',
    'San Antonio Villalongín',
    'San Baltazar Guelavila',
    'San Baltazar Temaxcalac',
    'San Baltazar Tetela',
    'San Bartolo Coyotepec',
    'San Bartolo Cuautlalpan',
    'San Bartolo Morelos',
    'San Bartolo Tutotepec',
    'San Bartolomé Cuahuixmatlac',
    'San Bernardo',
    'San Bernardo Tlalmimilolpan',
    'San Blas',
    'San Buenaventura',
    'San Buenaventura Nealtican',
    'San Buenaventura Tecaltzingo',
    'San Carlos Nuevo Guaymas',
    'San Cayetano',
    'San Ciro de Acosta',
    'San Clemente',
    'San Cosme Xalostoc',
    'San Cristóbal',
    'San Cristóbal de Ayala',
    'San Cristóbal de la Barranca',
    'San Cristóbal de las Casas',
    'San Cristóbal (El Cerrito)',
    'San Damián Texoloc',
    'San Diego',
    'San Diego de Alejandría',
    'San Diego de la Unión',
    'San Dionisio Ocotepec',
    'San Felipe',
    'San Felipe Aztatán',
    'San Felipe del Progreso',
    'San Felipe (El Chilarillo)',
    'San Felipe Guanajuato',
    'San Felipe Jalapa de Díaz',
    'San Felipe Orizatlán',
    'San Felipe Teotlalcingo',
    'San Felix Rijo',
    'San Fernando',
    'San Francisco',
    'San Francisco Cheje',
    'San Francisco Chimalpa',
    'San Francisco Coacalco',
    'San Francisco Cuapa',
    'San Francisco de Borja',
    'San Francisco de Conchos',
    'San Francisco de los Romo',
    'San Francisco de Rivas',
    'San Francisco del Mezquital',
    'San Francisco del Oro',
    'San Francisco del Rincón',
    'San Francisco Ixhuatán',
    'San Francisco Magu',
    'San Francisco Ocotlán',
    'San Francisco Periban',
    'San Francisco Tepeolulco',
    'San Francisco Tepeyecac',
    'San Francisco Xonacatlán',
    'San Francisco Zacacalco',
    'San Gabriel',
    'San Gabriel  Docuan',
    'San Gabriel Chilac',
    'San Gaspar de los Reyes',
    'San Germán',
    'San Ignacio',
    'San Ignacio Cerro Gordo',
    'San Ignacio Río Muerto',
    'San Isidro',
    'San Isidro Mazatepec',
    'San Jeronimito (San Jerónimo)',
    'San Jerónimo',
    'San Jerónimo Amanalco',
    'San Jerónimo Coyula',
    'San Jerónimo de Juárez',
    'San Jerónimo Tianguismanalco',
    'San Jerónimo Tlacochahuaya',
    'San Jerónimo Xayacatlán',
    'San Joaquín',
    'San José Acateno',
    'San José Alchichica',
    'San José Babicora',
    'San José Casas Caídas',
    'San José Chiltepec',
    'San José de Ahome',
    'San José de Aura',
    'San José de Bacum',
    'San José de Gracia',
    'San José de la Paz',
    'San José de las Moras',
    'San José de los Reynoso',
    'San José de Lourdes',
    'San José de Merino',
    'San José de Tuitán (Tuitán)',
    'San José del Cabo',
    'San José del Progreso',
    'San José del Rincón',
    'San José del Valle',
    'San José El Vidrio',
    'San José Huipana (Huipana)',
    'San José Iturbide',
    'San José Poliutla',
    'San José Temascatio',
    'San José Tilapa',
    'San José Tlautla',
    'San José Viejo',
    'San José Vistahermosa',
    'San Juan',
    'San Juan Amecac',
    'San Juan Atenco',
    'San Juan Bautista lo de Soto',
    'San Juan Bautista Valle Nacional',
    'San Juan Cacahuatepec',
    'San Juan Cosala',
    'San Juan Cuautlancingo',
    'San Juan de Abajo',
    'San Juan de Guadalupe',
    'San Juan de la Vega',
    'San Juan de los Lagos',
    'San Juan de Viña',
    'San Juan del Estado',
    'San Juan del Llanito',
    'San Juan del Río',
    'San Juan del Río del Centauro',
    'San Juan Epatlán',
    'San Juan Evangelista',
    'San Juan Guichicovi',
    'San Juan Ixcaquixtla',
    'San Juan Ixtayopan',
    'San Juan Mixtepec Distrito 08',
    'San Juan Pueblo Nuevo',
    'San Juan Tecomatlán',
    'San Juan Tetelcingo',
    'San Juan Tilapa',
    'San Juan Tilcuautla',
    'San Juan Tizahuapan',
    'San Juan Tlacotenco',
    'San Julián',
    'San Lorenzo Acopilco',
    'San Lorenzo Buenaventura',
    'San Lorenzo Cacaotepec',
    'San Lorenzo Chiautzingo',
    'San Lorenzo Cuapiaxtla',
    'San Lorenzo Dr. Belisario Dominguez',
    'San Lorenzo Malacota',
    'San Lucas',
    'San Lucas El Grande',
    'San Lucas Ojitlán',
    'San Lucas Tulcingo (Tulcingo)',
    'San Luis Acatlán',
    'San Luis Ayucan',
    'San Luis de la Loma',
    'San Luis de la Paz',
    'San Luis del Cordero',
    'San Luis Potosí',
    'San Luis Río Colorado',
    'San Luis Soyatlán',
    'San Luis Tecuahutitlán',
    'San Luis Tehuiloyocan',
    'San Marcos',
    'San Marcos Carmona',
    'San Marcos Contla',
    'San Marcos (Mpio. San Marcos)',
    'San Marcos (Mpio. Tonila)',
    'San Marcos (Mp.Zacoalco de Torres',
    'San Marcos Nepantla',
    'San Maria Atarasquillo',
    'San Martín',
    'San Martín Cachihuapan',
    'San Martín Chalchicuautla',
    'San Martín Cuautlalpan',
    'San Martín de las Flores',
    'San Martín de las Pirámides',
    'San Martín de Zula (Zula)',
    'San Martín Hidalgo',
    'San Martín Pachivia',
    'San Martín Peras',
    'San Martín Texmelucan',
    'San Martinito',
    'San Mateo',
    'San Mateo Atenco',
    'San Mateo Ayecac',
    'San Mateo Otzacatipan',
    'San Mateo Oxtotitlán',
    'San Mateo Tocuaro (Tocuaro)',
    'San Miguel',
    'San Miguel Ajusco',
    'San Miguel Canoa',
    'San Miguel Cuyutlán',
    'San Miguel de Allende',
    'San Miguel de Cruces',
    'San Miguel El Alto',
    'San Miguel Emenguaro',
    'San Miguel Epejan',
    'San Miguel Ixtapan',
    'San Miguel Jaltepec',
    'San Miguel Mimiapan',
    'San Miguel Octopan',
    'San Miguel Sola de Vega',
    'San Miguel Tecuixiapan',
    'San Miguel Tenextatiloyan',
    'San Miguel Tlaixpan',
    'San Miguel Topilejo',
    'San Miguel Totolapan',
    'San Miguel Xoxtla',
    'San Miguel Zapotitlán',
    'San Miguel Zinacantepec',
    'San Nicolás',
    'San Nicolás de Buenos Aires',
    'San Nicolás de Carretas',
    'San Nicolás de la Condesa',
    'San Nicolás de los Agustinos',
    'San Nicolás de los Garza',
    'San Nicolás de Presidio',
    'San Pablito',
    'San Pablo Amicano',
    'San Pablo Atlazalpan',
    'San Pablo Autopan',
    'San Pablo Balleza',
    'San Pablo Casacuarán',
    'San Pablo de las Tunas',
    'San Pablo Huitzo',
    'San Pablo Oztotepec',
    'San Pablo Pejo (San Pablo)',
    'San Patricio (Melaque)',
    'San Pedro',
    'San Pedro Amuzgos',
    'San Pedro Apóstol',
    'San Pedro Atlapulco',
    'San Pedro Atocpan',
    'San Pedro Benito Juárez',
    'San Pedro Buenavista',
    'San Pedro Comitancillo',
    'San Pedro Cuayuca',
    'San Pedro de la Cueva',
    'San Pedro de los Naranjos',
    'San Pedro de Tututepec',
    'San Pedro del Gallo',
    'San Pedro Garza García',
    'San Pedro Ixtlahuaca',
    'San Pedro Jácuaro (Las Joyas)',
    'San Pedro Lagunillas',
    'San Pedro los Baños',
    'San Pedro O Saucito',
    'San Pedro Piedra Gorda',
    'San Pedro Pochutla',
    'San Pedro Tapanatepec',
    'San Pedro Tenango El Viejo',
    'San Pedro Tesistán',
    'San Pedro Tlachichilco',
    'San Pedro Totolapa',
    'San Pedro Totoltepec',
    'San Pedro y San Pablo Teposco',
    'San Pedro Zacachimalpa',
    'San Quintín',
    'San Rafael',
    'San Rafael de Arriba',
    'San Rafael Tlanalapan',
    'San Ramón',
    'San Roque',
    'San Roque de Montes',
    'San Salvador',
    'San Salvador El Seco',
    'San Salvador El Verde',
    'San Salvador Huixcolotla',
    'San Salvador Torrecillas',
    'San Salvador Tzompantepec',
    'San Sebastián',
    'San Sebastián Aparicio',
    'San Sebastián del Alamo',
    'San Sebastián El Grande',
    'San Sebastián Tenochtitlán',
    'San Sebastián Villanueva',
    'San Simeon Xipetzinco',
    'San Simón de Guerrero',
    'San Simon Yehualtepec',
    'San Tadeo',
    'San Tiburcio',
    'San Vicente',
    'San Vicente Boquerón',
    'San Vicente Chicoloapán',
    'San Vicente de Juárez',
    'San Vicente Palapa',
    'San Vicente Tancuayalab',
    'Sánchez Celis',
    'Sánchez Celis (Gato de Lara)',
    'Sanctorum Lázaro Cárdenas',
    'Sandoval',
    'Santa Ana',
    'Santa Ana Ahuehuepan',
    'Santa Ana Amatlán (Santa Ana)',
    'Santa Ana de Allende',
    'Santa Ana del Aguila',
    'Santa Ana Hueytlalpan',
    'Santa Ana Jilotzingo',
    'Santa Ana Maya',
    'Santa Ana Nopalucan',
    'Santa Apolonia',
    'Santa Bárbara',
    'Santa Bárbara de Tutuaca',
    'Santa Catarina',
    'Santa Catarina Juquila',
    'Santa Catarina Yecahuitzotl',
    'Santa Clara',
    'Santa Clara del Cobre',
    'Santa Clara Ocoyucan',
    'Santa Cruz',
    'Santa Cruz Amilpas',
    'Santa Cruz Atizapán',
    'Santa Cruz Ayotuzco',
    'Santa Cruz de las Flores',
    'Santa Cruz de Miramar',
    'Santa Cruz de Rosales',
    'Santa Cruz del Astillero',
    'Santa Cruz del Valle',
    'Santa Cruz El Grande',
    'Santa Cruz Papalutla',
    'Santa Cruz Xitla',
    'Santa Cruz Xoxocotlán',
    'Santa Elena (Francisco Zarco)',
    'Santa Eulalia',
    'Santa Fe',
    'Santa Fe del Río',
    'Santa Gertrudis',
    'Santa Gertrudis (La Hacienda)',
    'Santa Inés Ahuatempan',
    'Santa Isabel',
    'Santa Lucía Cosamaloapan',
    'Santa María Ajoloapan',
    'Santa María Canchesda',
    'Santa María Chilapa de Díaz',
    'Santa María Coronango',
    'Santa María Cuevas (Cuevas)',
    'Santa María de Abajo',
    'Santa María del Monte',
    'Santa María del Oro',
    'Santa María del Río',
    'Santa María del Tule',
    'Santa María del Valle',
    'Santa María Huatulco',
    'Santa María Magdalena Cahuacán',
    'Santa María Mazatla',
    'Santa María Mixtequilla',
    'Santa María Moyotzingo',
    'Santa Maria Nativitas',
    'Santa María Tecomavaca',
    'Santa María Tlalmimilolpan',
    'Santa María Xadani',
    'Santa María Zacatepec',
    'Santa Matilde',
    'Santa Mónica',
    'Santa Mónica Ozumbilla',
    'Santa Rita',
    'Santa Rita Tlahuapan',
    'Santa Rosa',
    'Santa Rosa Jáuregui',
    'Santa Rosalía',
    'Santa Rosalía de Cuevas',
    'Santa Teresa',
    'Santander Jiménez',
    'Santas Marías',
    'Santiago',
    'Santiago Acutzilapan',
    'Santiago Alseseca',
    'Santiago Chazumba',
    'Santiago Coachochitlán',
    'Santiago Conguripo',
    'Santiago Cuautlalpan (Tepotzotlán)',
    'Santiago Cuautlalpan (Texcoco)',
    'Santiago de Anaya',
    'Santiago de Cuenda',
    'Santiago de la Peña',
    'Santiago Ixcuintla',
    'Santiago Jamiltepec',
    'Santiago Juxtlahuaca',
    'Santiago Maravatío',
    'Santiago Matatlán',
    'Santiago Miahuatlán',
    'Santiago Niltepec',
    'Santiago Oxtotitlán',
    'Santiago Papasquiaro',
    'Santiago Tangamandapio',
    'Santiago Tenango',
    'Santiago Tepeyahualco',
    'Santiago Tezontlale',
    'Santiago Tianguistenco',
    'Santiago Tingambato',
    'Santiago Tlapacoya',
    'Santiago Tlautla',
    'Santiago Tolman',
    'Santiago Tulantepec',
    'Santiago Tuxtla',
    'Santiago Undameo',
    'Santiago Yeche',
    'Santiago Yolomecatl',
    'Santiaguito de Velázquez',
    'Santo Domingo Atoyatempan',
    'Santo Domingo Chihuitán',
    'Santo Domingo Huehuetlán',
    'Santo Domingo Ingenio',
    'Santo Domingo Petapa',
    'Santo Domingo Tonalá',
    'Santo Domingo Yanhuitlán',
    'Santo Domingo Zanatepec',
    'Santo Niño Aguanaval',
    'Santo Tomás',
    'Santo Tomás Apipilhuasco',
    'Santo Tomás Chautla',
    'Santo Tomás Hueyotlipan',
    'Santos Reyes Nopala',
    'Sarabia',
    'Saric',
    'Sasabe',
    'Satevo',
    'Sauceda de la Borda',
    'Saucillo',
    'Sauta',
    'Sayula',
    'Sayula de Alemán',
    'Sección Enríquez',
    'Senguio',
    'Sentispac',
    'Sergio Butrón Casas',
    'Seybaplaya',
    'Seye',
    'Sihochac',
    'Silacayoapam',
    'Silao',
    'Simojovel',
    'Sinaloa de Leyva',
    'Singuilucan',
    'Sisoguichi',
    'Socoltenango',
    'Soledad de Doblado',
    'Soledad de Graciano Sánchez',
    'Soledad Etla',
    'Soltepec',
    'Sombrerete',
    'Sonoita',
    'Soto La Marina',
    'Sotuta',
    'Soyatitán',
    'Subteniente López',
    'Suchiapa',
    'Suchil',
    'Suchilapan del Río',
    'Sucila',
    'Sultepec',
    'Sumidero',
    'Suspiro Picacho',
    'Susupuato de Guerrero',
    'Tabasco',
    'Tacámbaro',
    'Tacoaleche',
    'Tacotalpa',
    'Tacubaya',
    'Tahmek',
    'Tala',
    'Talpa de Allende',
    'Tamalín',
    'Tamán',
    'Tamasopo',
    'Tamazula',
    'Tamazula de Victoria',
    'Tamazulita',
    'Tamazunchale',
    'Tambaca',
    'Tamiahua',
    'Tamos',
    'Tampamolón Corona',
    'Tampico',
    'Tampico Alto',
    'Tamuín',
    'Tamulte de las Sabanas',
    'Tancanhuitz',
    'Tancítaro',
    'Tangancícuaro',
    'Tanganhuato',
    'Tanhuato de Guerrero',
    'Tanlajas',
    'Tanquián de Escobedo',
    'Tantoyuca',
    'Tapachula',
    'Tapalpa',
    'Tapilula',
    'Tarandacuao',
    'Taretán',
    'Tarímbaro',
    'Tarimoro',
    'Tasinaxtla (La Cañada)',
    'Tasquillo',
    'Taxco',
    'Tayahua (San José de Tayahua)',
    'Tayoltita',
    'Teacapan',
    'Teapa',
    'Tecali de Herrera',
    'Tecalitlán',
    'Tecamachalco',
    'Tecate',
    'Tecoanapa',
    'Tecoh',
    'Tecolotlán',
    'Tecolutilla',
    'Tecolutla',
    'Tecomán',
    'Tecomatlán',
    'Tecozautla',
    'Tecpan de Galeana',
    'Tecpatán',
    'Tecuala',
    'Tecualtitán',
    'Tehuacán',
    'Tehuantepec',
    'Tehuetlán',
    'Tehuitzingo',
    'Tehuixtla',
    'Tejupilco de Hidalgo',
    'Tekanto',
    'Tekax',
    'Tekit',
    'Telchac Pueblo',
    'Telchac Puerto',
    'Telixtac',
    'Teloloapan',
    'Temalacatzingo',
    'Temascal',
    'Temascalapa',
    'Temascalcingo',
    'Temascaltepec',
    'Temax',
    'Temixco',
    'Temoaya',
    'Temoris',
    'Temosachi',
    'Temozón',
    'Tempoal de Sánchez',
    'Tenabo',
    'Tenamaxtlán',
    'Tenampulco',
    'Tenancingo',
    'Tenango de Arista',
    'Tenango de Doria',
    'Tenango del Aire',
    'Tenantongo (Avándaro)',
    'Tenejapa',
    'Tenenexpan',
    'Tenexpa',
    'Tenextepango',
    'Tenjay',
    'Tenosique',
    'Teñhe',
    'Teocalco',
    'Teocaltiche',
    'Teocelo',
    'Teocuitatlán de Corona',
    'Teoloyucan',
    'Teopantlán',
    'Teopisca',
    'Teotihuacán',
    'Teotitlán de Flores Magón',
    'Teotitlán del Valle',
    'Tepache',
    'Tepalcatepec',
    'Tepalcingo',
    'Tepanco de López',
    'Tepatepec',
    'Tepatitlan de Morelos',
    'Tepatlaxco de Hidalgo',
    'Tepeaca',
    'Tepechitlán',
    'Tepecoacuilco de Trujano',
    'Tepehuanes',
    'Tepeji de Ocampo',
    'Tepetitlán',
    'Tepetixtla',
    'Tepetlán',
    'Tepetlaoxtoc',
    'Tepetzintla',
    'Tepexco',
    'Tepexi de Rodríguez',
    'Tepexpan',
    'Tepezala',
    'Tepic',
    'Tepojaco',
    'Tepotzotlán',
    'Tepoztlán',
    'Tepusco',
    'Tepuzhuacán',
    'Tequesquitengo',
    'Tequesquitlán',
    'Tequexquinahuac',
    'Tequexquitla',
    'Tequila',
    'Tequisquiapan',
    'Tequixquiac',
    'Tesistán',
    'Tetecala',
    'Tetela',
    'Tetela de Ocampo',
    'Tetela del Volcán',
    'Teteles',
    'Tetillas',
    'Tetitlán',
    'Tetla',
    'Teuchitlán',
    'Teul de González Ortega',
    'Texcaltitlán',
    'Texcoco de Mora',
    'Texistepec',
    'Teziutlán',
    'Tezoatlán de Segura y Luna',
    'Tezonapa',
    'Tezontepec de Aldama',
    'Tezoyuca',
    'Tianguistengo',
    'Ticuitaco (Tocuitaco)',
    'Ticul',
    'Ticumán',
    'Tierra Blanca',
    'Tierra Colorada',
    'Tierra y Libertad',
    'Tierranueva',
    'Tihuatlán',
    'Tijuana',
    'Tila',
    'Tilzapotla',
    'Tinaja de Vargas',
    'Tinguindín',
    'Tiquicheo',
    'Tiripetio',
    'Tixkokob',
    'Tixtla',
    'Tizapán El Alto',
    'Tizayuca',
    'Tizimín',
    'Tlachichila',
    'Tlachichuca',
    'Tlacoachistlahuaca',
    'Tlacojalpan',
    'Tlacolula',
    'Tlacolula de Matamoros',
    'Tlacotalpan',
    'Tlacote El Bajo',
    'Tlacotepec',
    'Tlacuitapan',
    'Tlahualilo de Zaragoza',
    'Tlahuelilpan',
    'Tlahuelompa',
    'Tlajomulco de Zuñiga',
    'Tlalchapa',
    'Tlalixcoyan',
    'Tlalixtaquilla',
    'Tlalmanalco',
    'Tlalnepantla',
    'Tlalpujahua',
    'Tlaltenango',
    'Tlaltizapan',
    'Tlanchinol',
    'Tlapa de Comonfort',
    'Tlapacoyan',
    'Tlapanaloya',
    'Tlapehuala',
    'Tlaquepaque',
    'Tlaquiltenango',
    'Tlatlauquitepec',
    'Tlaxcala',
    'Tlaxco',
    'Tlaxcoapan',
    'Tlaxiaco',
    'Tlaxmalac',
    'Tlayacapan',
    'Tlayecac',
    'Tlazala de Fabela',
    'Tlazazalca',
    'Tochimilco',
    'Tochtepec',
    'Tocumbo',
    'Todos Santos',
    'Tolcayuca',
    'Tolimán',
    'Tolome',
    'Toluca',
    'Tomatlán',
    'Tomelopitos',
    'Tomochi',
    'Tonalá',
    'Tonantzintla',
    'Tonatico',
    'Tonaya',
    'Tonila',
    'Topia',
    'Topolobampo',
    'Torreón',
    'Torreón de Cañas',
    'Totatiche',
    'Totolapan',
    'Tototlan',
    'Totutla',
    'Trancoso',
    'Transfiguración',
    'Trapiche del Rosario',
    'Trejos',
    'Tres Alamos',
    'Tres Picos',
    'Tres Valles',
    'Tres Zapotes',
    'Trincheras',
    'Trinidad de Viguera',
    'Tronconal',
    'Troncones (Emiliano Zapata)',
    'Tula',
    'Tulancingo',
    'Tulcingo de Valle',
    'Tulimán',
    'Tultepec',
    'Tultitlán de Mariano Escobedo',
    'Tulum',
    'Tumbiscatio de Ruiz',
    'Tungareo',
    'Tunkas',
    'Turitzio',
    'Tuxcacuesco',
    'Tuxpan',
    'Tuxtepec',
    'Tuxtilla',
    'Tuxtla Chico',
    'Tuxtla Gutiérrez',
    'Tuzamapan',
    'Tuzantla',
    'Tzicatlán',
    'Tzimol',
    'Tzintzingareo',
    'Tziritzicuaro (Nativitas)',
    'Tzucacab',
    'Uman',
    'Unión de Guadalupe',
    'Unión de San Antonio',
    'Unión de Tula',
    'Unión Hidalgo',
    'Unión Juárez',
    'Ures',
    'Uriangato',
    'Urique',
    'Ursulo Galván',
    'Uruachi',
    'Uruapan',
    'Uruetaro',
    'Uspero (Reynosa)',
    'Uxmal  (Hoteles)',
    'Valadeces',
    'Valentín Gómez Farías',
    'Valladolid',
    'Valle de Banderas',
    'Valle de Bravo',
    'Valle de Chalco',
    'Valle de Guadalupe',
    'Valle de Ignacio Allende',
    'Valle de Juárez',
    'Valle de Juárez (Jerahuaro)',
    'Valle de la Trinidad',
    'Valle de las Palmas',
    'Valle de Santiago',
    'Valle de Zaragoza',
    'Valle Hermoso',
    'Valparaíso',
    'Valtierrilla',
    'Vanegas',
    'Vega de Alatorre',
    'Veinte de Noviembre',
    'Velardeña',
    'Venado',
    'Vencedores',
    'Venustiano Carranza',
    'Venustiano Carranza (Cualata)',
    'Veracruz',
    'Vicam (Switch)',
    'Vicente Camalote',
    'Vicente Guerrero',
    'Vicente Guerrero (Algodones)',
    'Victoria',
    'Viesca',
    'Villa  Angel Flores',
    'Villa Adolfo López M. (El Tamar',
    'Villa Ahumada y Anexas',
    'Villa Alberto A. Alvarado A.',
    'Villa Aldama',
    'Villa Aldama 1a. Sección',
    'Villa Avila Camacho (La Ceiba)',
    'Villa Azueta',
    'Villa Benito Juárez',
    'Villa Cacalilao (Cacalilao)',
    'Villa Corona',
    'Villa Corzo',
    'Villa de Acala',
    'Villa de Arista',
    'Villa de Arriaga',
    'Villa de Contla (Contla)',
    'Villa de Cos',
    'Villa de Etla',
    'Villa de Juárez',
    'Villa de las Rosas',
    'Villa de Méndez',
    'Villa de Paras',
    'Villa de Ramos',
    'Villa de Reyes',
    'Villa de Tamazulapam del Progre',
    'Villa del Carbón',
    'Villa Emiliano Zapata',
    'Villa Flores',
    'Villa Guerrero',
    'Villa Gustavo Díaz O (Campo',
    'Villa Hermosa',
    'Villa Hidalgo',
    'Villa Hidalgo (El Cubo)',
    'Villa Hidalgo (El Nuevo)',
    'Villa Insurgentes',
    'Villa Jiménez',
    'Villa Juárez',
    'Villa Juárez (La Trozada)',
    'Villa Juárez(Campo Gob.)',
    'Villa Lázaro Cárdenas',
    'Villa Lázaro Cárdenas (La Uno)',
    'Villa López',
    'Villa Madero',
    'Villa Manuel',
    'Villa Mariano Matamoros',
    'Villa Morelos',
    'Villa Nicolás Bravo',
    'Villa Nicolás Romero',
    'Villa Ocampo',
    'Villa Orestes Pereyra',
    'Villa Purificación',
    'Villa Rafael Lara Grajales',
    'Villa Tejeda',
    'Villa Tezontepec',
    'Villa Unión',
    'Villa Vicente Guerrero',
    'Villa Victoria',
    'Villagrán',
    'Villahermosa',
    'Villamar',
    'Villanueva',
    'Villas Campestres',
    'Villas de Irapuato',
    'Vista Hermosa',
    'Vizarrón de Montes',
    'Xala',
    'Xalatlaco',
    'Xalisco',
    'Xaloxtoc',
    'Xaltianguis',
    'Xaltocan',
    'Xcaret',
    'Xico',
    'Xicoténcatl',
    'Xicotepec de Juárez',
    'Xilitla',
    'Xmatkuil',
    'Xochihuehuetlán',
    'Xochipala',
    'Xochistlahuaca',
    'Xochitepec',
    'Xochitlán',
    'Xometla',
    'Xoxocotla',
    'Xpu-ha',
    'Xpujil',
    'Yahualica',
    'Yajalón',
    'Yanga',
    'Yautepec',
    'Yávaros (Isla Las Viejas)',
    'Yecapixtla',
    'Yecora',
    'Yecuatla',
    'Yolotepec',
    'Yoricostio (La Villita)',
    'Yurécuaro',
    'Yuriria',
    'Yustis',
    'Zaachila',
    'Zacacoyuca',
    'Zacamulpa',
    'Zacapala',
    'Zacapalco',
    'Zacapoaxtla',
    'Zacapu',
    'Zacapuato',
    'Zacate Colorado',
    'Zacatecas',
    'Zacatelco',
    'Zacatepec de Hidalgo',
    'Zacatipa',
    'Zacatlán',
    'Zacoalco',
    'Zacualpan',
    'Zacualtipán',
    'Zamora',
    'Zapopan',
    'Zapote de Adjuntas',
    'Zapote de Peralta',
    'Zapote del Valle',
    'Zapotiltic',
    'Zapotitán de Hidalgo',
    'Zapotitlán de Vadillo',
    'Zapotitlán Lagunas',
    'Zapotitlán Salinas',
    'Zapotitos',
    'Zapotlán de Juárez',
    'Zapotlán del Rey',
    'Zapotlanejo',
    'Zaragoza',
    'Zempoala',
    'Zicuiran',
    'Zihuatanejo',
    'Zimapán',
    'Zimatlán de Alvarez',
    'Zinaparo',
    'Zinapécuaro',
    'Ziquitaro',
    'Ziracuaretiro',
    'Zirahuén',
    'Zirándaro',
    'Zitacuaro',
    'Zitlaltepec',
    'Zongolica',
    'Zozocolco de Hidalgo',
    'Zumpahuacán',
    'Zumpango',
    'Zumpango del Río',
];
